    ...
    cargarNotas() {
        ...
        if (NotasACargar > 0) {
            for (let i = 0; i < NotasACargar; i++) {
                ...
                //localStorage.setItem("notas", [this.notaParcial12y3])
                localStorage.setItem(`${this.nombre}_${this.apellido}_notas`, JSON.stringify(this.notaParcial12y3));

                }
                // document.getElementById("notas123").innerText = this.notaParcial12y3[0] +" "+ this.notaParcial12y3[1] +" "+ this.notaParcial12y3[2];
            }
